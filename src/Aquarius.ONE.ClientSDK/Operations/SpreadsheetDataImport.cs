...
namespace ONE.Operations
{
    public class SpreadsheetDataImport
    {
        ...
        private CloneOperation _cloneOperation;
        ...
        public SpreadsheetDataImport(ClientSDK clientSDK, OperationCache operationCache, string operationId, string tenantId)
        {
        ...
        public async Task<bool> ImportAsync(EnumWorksheet worksheetType, string json)
        {
            ...
            {
                _columnTwins = await _clientSDK.DigitalTwin.GetDescendantsByTypeAsync(_operationId, Constants.TelemetryCategory.ColumnType.RefId);
            }
            _columnMap = await _cloneOperation.GetSourceToDestinationColumnMap(_operationId);

            if (_userTwins == null)
                _userTwins = await _clientSDK.DigitalTwin.GetDescendantsByTypeAsync(_tenantId, Constants.OrganizationCategory.UserType.RefId);
            if (_users == null)
        ...
        private async Task<Cell> ConvertCellAsync(Cell exportedCell, WorksheetDefinition worksheetDefinition)
        {
            ...
            var column = ONE.Operations.Spreadsheet.Helper.GetColumnByNumber(worksheetDefinition, cell.ColumnNumber);
